...
install_go() {
    if [ x"$ARCH" = "xx86_64" ]; then
        until wget https://go.dev/dl/go1.26.3.linux-amd64.tar.gz; do
        ...
        until wget https://go.dev/dl/go1.26.3.linux-arm64.tar.gz; do
            echo "Service not ready, retrying in 10 seconds..."
            sleep 10
        done
        rm -rf /usr/local/go
